...
function TeacherResult() {
  ...
  useEffect(() => {
    axios
      .get(`${API_BASE_URL}:8000/teacher/chat/result/${chatroom_id}`, {
        headers: {
  ...
  const formattedDate = new Date(resultData.result_time)
    ...
    <div className="TR-fullbox">
      <div className="TR-contentbox">
        <div className="TR-dateBar">
          ...
          <div className="TR-date">{formattedDate}</div>
        </div>
        {userData.username && (
          <div className="TR-title">
            <img className="TR-title-profile" src={`${API_BASE_URL}:8000` + userData.profile_photo} />
            <div className="TR-title-nickname">{userData.username}</div>
            <div
              className={`TR-title-circle ${
                emotion === '행복' ? 'happy' : emotion === '우울' ? 'sad' : emotion === '평범' ? 'normal' : ''
              }`}
            ></div>
            <div className="TR-title-keywords">{resultData.category}</div>
          </div>
        )}
          ...
          <div className="Result1-clipboard">
            ...
            <div className="Result1-chart">
              ...
              <ApexChart
                ...
                width={320}
              />
              <span className="R1-Hp">{Math.round(100 - resultData.emotion_temp)}%</span>
              <span className="R1-Gp">{Math.round(resultData.emotion_temp)}%</span>
            </div>
          </div>
        </div>
        <div className="Result2-detail">
          ...
          <div className="Result2-box">
            <ApexChart options={options2} series={seriesData} type="pie" height={370} />
          </div>
        </div>
        <div className="Result3-keyword">
          ...
          <div className="Result3-box">
            {resultData.wordcloud && (
              <img
                src={`${API_BASE_URL}:8000` + resultData.wordcloud}
                alt="Wordcloud"
                className="Result3-wordcloud"
              />
            )}
          </div>
        </div>
        <div className="Result4-summary">
          ...
          <div className="Result4-box">
            <div className="Result4-box2">{resultData.summary}</div>
          </div>
        </div>
