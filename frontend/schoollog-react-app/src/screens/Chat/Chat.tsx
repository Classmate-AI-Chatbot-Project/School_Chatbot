...
function Chat() {
  ...
  const handleChatAnswer = (id: number, answer?: string) => {
    setChatLog((prevChatLog) =>
      prevChatLog.map((chat) => (chat.id === id ? { ...chat, isLoading: false, answer: answer } : chat))
    );
  };
        ...
        <div className="Chat-Input">
          {isToggleOpen && (
            <div className={`Chat-Toggle ${isToggleOpen ? 'active' : ''}`}>
              ...
              <button className="Chat-ToggleButton2" onClick={openUsageModal}>
                이용방법
              </button>
              <button className="Chat-ToggleButton2" onClick={openNoticeModal}>
                유의사항
              </button>
            </div>
          )}
