...
const SchoolSearchModal: React.FC<SchoolSearchModalProps> = ({ modalWidth, onSelectSchool, onClose }) => {
  ...
  const filteredSchools = schoolList.filter((school) => school.SCHUL_NM.includes(searchQuery));
  ...
  const getSchoolList = async () => {
    try {
      axios
        .get(
          `https://open.neis.go.kr/hub/schoolInfo?KEY=${key}&Type=json&pIndex=1&pSize=100&SCHUL_NM=${searchQuery}&SCHUL_KND_SC_NM=${selectedSchoolType}`
        )
        .then((response) => {
  ...
  return (
    <div
      className="SchoolSearchModal-fullbox"
      style={{
        width: modalWidth,
      }}
    >
      ...
      <div className="SchoolSearchModal-header">
        ...
        <div className="SchoolSearchModal-searchbox">
          ...
          />
          <button onClick={getSchoolList}>검색</button>
        </div>
      </div>
      ...
      <div className="SchoolSearchModal-content">
        ...
        <button className="SchoolSearchModal-confirmbtn" onClick={handleConfirm}>
          확인
        </button>
      </div>
    </div>
