...
function SideBar({ isOpen, setIsOpen }: { isOpen: boolean; setIsOpen: any }) {
  ...
  useEffect(() => {
    if (isLoggedIn) {
      axios
        .get(`${API_BASE_URL}:8000/consult/check_unread_messages/`, {
          headers: {
            'Content-type': 'application/json',
          },
          withCredentials: true,
  ...
  const gotoChat = () => {
    if (!isLoggedIn) {
      ...
      // user_id가 있으면 ChatRoom 생성 및 이동
      axios
        .post(`${API_BASE_URL}:8000/chat/create/`, null, {
          headers: {
            'Content-type': 'application/json',
    ...
    <div className={isOpen ? 'Opened-bar' : 'Closed-bar'} ref={outside}>
      {isLoggedIn && userData.job === 'Teacher' ? (
        <ul>
          <li>
            <SpeechIcon />
            <a className="Menu-item-text" onClick={gotoTeacherChatList}>
              상담 대화방 목록
            </a>
            {isUnread && <NewIcon />}
          </li>
          ...
          <li>
            <SpeechIcon />
            <a className="Menu-item-text" onClick={gotoConsultResulList}>
              챗봇 대화 분석 결과 리스트
            </a>
            {hasNewResult && <NewIcon />}
          </li>
          <BorderLine width={'315px'} height={'1px'} />
          <li>
            <StudentsIcon />
            <a className="Menu-item-text" onClick={gotoStudentList}>
              상담 학생 목록
            </a>
          </li>
        ...
        <ul>
          <li>
            <PaintingIcon />
            <a className="Menu-item-text" onClick={gotoTest}>
              그림 심리 테스트
            </a>
          </li>
          <BorderLine width={'315px'} height={'1px'} />
          <li>
            <SpeechIcon />
            <a className="Menu-item-text" onClick={gotoChat}>
              챗봇과 상담하기
            </a>
          </li>
          <BorderLine width={'315px'} height={'1px'} />
          <li>
            <StudentsIcon />
            <a className="Menu-item-text" onClick={gotoConsult}>
              선생님과 상담하기
            </a>
            {isUnread && <NewIcon />}
          </li>
